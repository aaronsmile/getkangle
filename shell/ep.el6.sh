...
DOWNLOAD_BASE_URL="http://github.itzmx.com/bangteng/getkangle/master";
restore_config()
{
...
function setup_kangle
{
	...
	wget --no-check-certificate $KANGLE_URL -O kangle-$KANGLE_VERSION.tar.gz
	if [ $? != 0 ] ; then
		exit $?
	fi
...
function setup_pureftpd
{
	...
	del_proftpd
	DOWN_URL="https://download.pureftpd.org/pub/pure-ftpd/releases/pure-ftpd-$PUREFTP_VERSION.tar.bz2"
	WGET_NEW_NAME="pure-ftpd-$PUREFTP_VERSION.tar.bz2"
...
wget http://github.itzmx.com/bangteng/getkangle/master/easypanel/main.php -O /vhs/kangle/nodewww/dbadmin/mysql/main.php
$PREFIX/bin/kangle -q
killall -9 kangle
sleep 3
$PREFIX/bin/kangle
